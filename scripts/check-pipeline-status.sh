...
# default branch it would otherwise vanish into a grey run (issue #9).
#
# Environment:
...
# Native-PHP template: the JSON filter runs through php -r, mirroring how the
# js template filters through node and the python template through python3.
select_by_result() {
  NEEDS_JSON="$NEEDS_JSON" WANT_RESULT="$1" php -r '
    ...
    foreach ($needs as $name => $job) {
        if (($job["result"] ?? null) === getenv("WANT_RESULT")) {
            $names[] = (string) $name;
        }
    }
    echo implode(", ", $names);
